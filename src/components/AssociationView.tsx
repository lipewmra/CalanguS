...
export const ROLE_PAYMENTS: Record<string, string> = {
  ...
  "Tradutor-Intérprete de Libras": "R$ 448,91",
  "Interprete de Libras": "R$ 448,91",
  "Guia-Intérprete de Surdocegos": "R$ 535,91",
  "Guia-Intérprete": "R$ 535,91",
  "Guia-Interprete": "R$ 535,91",
  "Ledor (Aplicador Especializado)": "R$ 361,91",
  ...
  "Fiscal Volante / Corredor": "R$ 180,00",
  "Fiscal Volante": "R$ 180,00",
  "Técnico de Informática": "R$ 240,00",
...
export const OFFICIAL_PAYMENT_LIST = [
  { role: "Guia-Intérprete de Surdocegos", value: "R$ 535,91", rawValue: 535.91, desc: "Atendimento especializado a surdocegos em trio (Tadoma/Libras Tátil)" },
  { role: "Tradutor-Intérprete de Libras", value: "R$ 448,91", rawValue: 448.91, desc: "Atendimento especializado em dupla a participantes usuários de Libras" },
  { role: "Ledor (Aplicador Especializado)", value: "R$ 361,91", rawValue: 361.91, desc: "Atendimento especializado em dupla para leitura de prova e enunciados" },
  { role: "Transcritor (Aplicador Especializado)", value: "R$ 361,91", rawValue: 361.91, desc: "Atendimento especializado individual para transcrição de respostas e redação" },
  { role: "Representante do Local", value: "R$ 272,20", rawValue: 272.20, desc: "Apoio de infraestrutura predial e ligação operacional do local" },
  { role: "Chefe de Sala", value: "R$ 240,00", rawValue: 240, desc: "Abertura de malotes, vistorias, atas oficiais e cronometragem" },
  { role: "Técnico de Informática", value: "R$ 240,00", rawValue: 240, desc: "Suporte especializado a salas com Videoprova em Libras ou leitor de tela" },
  { role: "Aplicador", value: "R$ 180,00", rawValue: 180, desc: "Distribuição e fiscalização direta em sala de prova com detector" },
  { role: "Fiscal Volante / Corredor", value: "R$ 180,00", rawValue: 180, desc: "Circulação em corredores, condução e apoio de trânsito" },
  { role: "Fiscal de Banheiro", value: "R$ 180,00", rawValue: 180, desc: "Inspeção e vistoria eletrônica nos sanitários com detector" },
  { role: "Auxiliar de Limpeza", value: "R$ 170,00", rawValue: 170, desc: "Higienização contínua de salas e sanitários" },
  { role: "Porteiro", value: "R$ 170,00", rawValue: 170, desc: "Controle de acesso e fechamento pontual de portões às 13h" },
];
...
export function getRolePayment(roleName: string): string {
  ...
  if (lower.includes("guia") || lower.includes("surdocego")) return "R$ 535,91";
  if (lower.includes("libras")) return "R$ 448,91";
  if (lower.includes("ledor") || lower.includes("leitor") || lower.includes("transcritor")) return "R$ 361,91";
  ...
  if (lower.includes("volante") || lower.includes("corredor")) return "R$ 180,00";
...
export default function AssociationView({ 
  ...
  // Auto-calculate suggested targets based on building rooms & SuperAdmin metrics
  ...
  // Collapse state for target quantities: starts collapsed if configured, expanded if not configured
  const hasConfiguredTargets = React.useMemo(() => {
    if (!building?.rolesTargetQuantities) return false;
    return Object.values(building.rolesTargetQuantities).some(qty => Number(qty) > 0);
  }, [building?.rolesTargetQuantities]);

  const [showTargetQuantitiesForm, setShowTargetQuantitiesForm] = useState<boolean>(() => {
    if (building?.rolesTargetQuantities && Object.values(building.rolesTargetQuantities).some(qty => Number(qty) > 0)) {
      return false;
    }
    return true;
  });
  ...
  useEffect(() => {
    if (building?.rolesTargetQuantities && !initialLoadRef.current) {
      const hasAny = Object.values(building.rolesTargetQuantities).some(qty => Number(qty) > 0);
      setShowTargetQuantitiesForm(!hasAny);
      if (hasAny) {
  ...
  // Initialize target quantities from building data (or fallback to autoCalculatedTargets if empty)
  useEffect(() => {
    const initial: Record<string, number> = {};
    const hasCustomTargets = building?.rolesTargetQuantities && Object.values(building.rolesTargetQuantities).some(qty => Number(qty) > 0);

    activeRoles.forEach(r => {
      if (hasCustomTargets && building?.rolesTargetQuantities?.[r.name] !== undefined) {
        initial[r.name] = Number(building.rolesTargetQuantities[r.name]) || 0;
      } else if (autoCalculatedTargets[r.name] !== undefined) {
        initial[r.name] = autoCalculatedTargets[r.name];
      } else {
        initial[r.name] = r.targetQuantity || 0;
      }
    });
    setTargetQuantities(initial);
  }, [building, activeRoles, autoCalculatedTargets]);

  // Handle auto-apply metrics calculation
  const handleApplySuperAdminMetrics = () => {
    if (!building) return;
    setTargetQuantities(prev => {
      const updated = { ...prev };
      activeRoles.forEach(r => {
        if (autoCalculatedTargets[r.name] !== undefined) {
          updated[r.name] = autoCalculatedTargets[r.name];
        }
      });
      return updated;
    });
    setSuccessMsg("Metas recalculadas automaticamente com base no quantitativo de salas do prédio e diretrizes do SuperAdmin!");
  ...
  // Filtered Collaborators from approved list across any available fields
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ========================================================================= */}
      {/* 1ª DIV: VER COBERTURA DE CARGOS E VALORES DO PRÉDIO */}
      {/* ========================================================================= */}
      <details open className="bg-slate-50 dark:bg-[#070b13]/60 border-2 border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-xs cursor-pointer group">
        <summary className="font-extrabold text-sm text-indigo-700 dark:text-indigo-400 select-none flex items-center justify-between focus:outline-hidden hover:text-indigo-600">
          <div className="flex items-center gap-2.5">
            <span className="p-2 bg-indigo-500/10 rounded-xl text-indigo-600 dark:text-indigo-400">📊</span>
            <div>
              <span className="font-black text-slate-850 dark:text-white">Ver Cobertura de Cargos e Valores do Prédio</span>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium mt-0.5">
                Acompanhe o preenchimento de cada vaga planejada e o valor correspondente
              </p>
            </div>
          </div>
          <span className="text-[10px] font-black uppercase text-slate-400 bg-slate-100 dark:bg-slate-800 px-3 py-1 rounded-full group-open:hidden">Abrir Resumo</span>
          <span className="text-[10px] font-black uppercase text-indigo-500 bg-indigo-500/10 px-3 py-1 rounded-full hidden group-open:inline-block">Fechar Resumo</span>
        </summary>

        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 gap-3 mt-4 pt-4 border-t border-slate-200 dark:border-slate-800 cursor-default">
      ...
      {/* ========================================================================= */}
      {/* 2ª DIV: LISTA COM O NOME DOS COLABORADORES E FILTRO (MATRIZ DE ASSOCIAÇÃO) */}
      {/* ========================================================================= */}
      <div className="bg-white dark:bg-[#101726]/80 border-2 border-slate-150 dark:border-slate-800 rounded-3xl p-5 shadow-xs">
        ...
        <div className="flex flex-col md:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Buscar por qualquer campo (nome, CPF, e-mail, WhatsApp, PIX, cargo, sala, indicação...)"
              value={searchTerm}
              ...
              return (
                <div 
                  ...
                  className={`p-4 border-2 rounded-2xl transition-all duration-150 flex flex-col justify-between ${
                    isAssigned 
                      ...
                      : "bg-emerald-50/70 dark:bg-emerald-950/25 border-emerald-300 dark:border-emerald-800/80 shadow-[2px_2px_0px_0px_rgba(16,185,129,0.08)]"
                ...
                >
                  <div>
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-2.5 min-w-0 flex-1">
                        ...
                        <div className="truncate min-w-0 flex-1">
                          ...
                          <p className="text-[10px] text-slate-500 dark:text-slate-400 font-mono font-bold mt-0.5 truncate">
                    ...
                    {/* Metadata attributes */}
                    <div className={`mt-2.5 space-y-1 rounded-xl p-2.5 border text-[10px] ${
                      isAssigned
                        ? "bg-white/80 dark:bg-[#070b13]/80 border-rose-200/60 dark:border-rose-900/40"
                        : "bg-white/80 dark:bg-[#070b13]/80 border-emerald-200/60 dark:border-emerald-900/40"
                    }`}>
                  ...
                  <div className={`mt-4 pt-3.5 border-t ${isAssigned ? "border-rose-200/60 dark:border-rose-900/40" : "border-emerald-200/60 dark:border-emerald-900/40"}`}>
                    <div className="flex items-center justify-between mb-1.5">
                      <label className="text-[9px] uppercase font-black text-slate-400 tracking-wider">
                        Designar Função do ENEM:
                      </label>
                      {collab.assignedRole && (
                        <span className="text-[9.5px] font-mono font-black text-rose-600 dark:text-rose-400">
                    ...
                    {collab.assignedRoom ? (
                      <p className="text-[9px] text-rose-600 dark:text-rose-400 mt-1.5 font-bold flex items-center gap-1">
                        <span>🚪</span> Alocado na <strong>{collab.assignedRoom}</strong>
                      </p>
                    ) : null}
                  </div>
                </div>
      ...
      {/* ========================================================================= */}
      {/* 3ª DIVS EM SEGUIDA: RESUMO ESTATÍSTICO, VALORES OFICIAIS & PLANEJAMENTO */}
      {/* ========================================================================= */}
      ...
      {/* TABELA OFICIAL DE REMUNERAÇÃO POR CARGO */}
      ...
      {/* DIV DE QUANTITATIVOS: Planning of Target Quantities Card */}
      <div className="bg-white dark:bg-[#101726]/80 rounded-3xl border-2 border-slate-150 dark:border-slate-800 p-5 shadow-xs relative overflow-hidden">
        <div className="flex items-center justify-between cursor-pointer select-none" onClick={() => setShowTargetQuantitiesForm(!showTargetQuantitiesForm)}>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-slate-800 dark:text-white">
                📋 Planejamento de Metas de Contratação por Função & Quantitativos
              </h3>
              <p className="text-[10px] text-slate-400 font-bold mt-0.5">
                {hasConfiguredTargets 
                  ? "Quantitativos configurados (clique para expandir ou alterar metas)" 
          ...
          >
            {showTargetQuantitiesForm ? (
              ...
              <>Configurar Quantitativos <ChevronDown className="w-3.5 h-3.5" /></>
            )}
          </button>
        </div>
        ...
        {showTargetQuantitiesForm && (
          <form onSubmit={handleSaveTargets} className="mt-5 pt-4 border-t border-slate-150 dark:border-slate-850 space-y-4 animate-fade-in">
            {!building ? (
              ...
              <div className="p-3.5 bg-emerald-500/5 dark:bg-emerald-950/20 border-2 border-emerald-500/20 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                <div className="space-y-0.5">
                  <div className="flex items-center gap-1.5 font-bold text-emerald-800 dark:text-emerald-300">
                    <Calculator className="w-4 h-4 text-emerald-600 dark:text-emerald-400 shrink-0" />
                    <span>Inventário de Salas: <strong>{building.roomsCount || 0} Regulares</strong> • <strong>{building.specialRoomsCount || 0} Especiais</strong> • <strong>{building.extraRoomsCount || 0} Extras</strong></span>
                  </div>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400">
                    As metas abaixo foram pré-calculadas a partir das diretrizes de dimensionamento do SuperAdmin. Você pode fazer ajustes para mais ou menos livremente.
                  </p>
                </div>

                <button
                  type="button"
                  onClick={handleApplySuperAdminMetrics}
                  className="px-3.5 py-2 rounded-xl text-[11px] font-black bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm flex items-center gap-1.5 transition shrink-0 cursor-pointer active:scale-95"
                  title="Recalcular automaticamente as metas com base no número de salas e regras do SuperAdmin"
                >
                  <Sparkles className="w-3.5 h-3.5 text-yellow-300" />
                  <span>Recalcular pelas Métricas</span>
                </button>
              </div>
            )}
